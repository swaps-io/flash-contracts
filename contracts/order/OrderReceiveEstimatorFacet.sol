...
import {IOrderReceiveEstimator} from "./interfaces/IOrderReceiveEstimator.sol";

import {OrderHashLib, Order} from "./OrderHashLib.sol";
import {OrderReceiverLib} from "./OrderReceiverLib.sol";

contract OrderReceiveEstimatorFacet is IOrderReceiveEstimator, Estimator {
    using SafeERC20 for IERC20;

    function estimateReceiveOrderAsset(Order calldata order_, bytes calldata fromSignature_, address caller_) external onlyEstimate {
        ...
        OrderReceiverLib.store().collateralLocker.commitLock(caller_, order_.collateralAmount, order_.collateralChain, order_.collateralUnlocked);
