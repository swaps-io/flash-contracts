...
interface IOrderReceiveManualNativeEstimator is IEstimator, IOrderReceiverManualNativeErrors, IOrderReceiverErrors, IOrderReceiverEvents {
    function estimateReceiveOrderAssetManualNative(Order calldata order, bytes calldata toSignature, address caller) external payable;

    function estimateReceiveOrderAssetManualNative(Order calldata order, bytes calldata toSignature, address caller, uint256 value) external payable;
}
