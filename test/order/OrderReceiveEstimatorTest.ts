...
import { VoidSigner, ZeroAddress, parseEther } from 'ethers';
...
describe('OrderReceiveEstimatorTest', function () {
  async function deployFixture() {
    ...
    const OrderReceiveEstimatorFacet = await ethers.getContractFactory('OrderReceiveEstimatorFacet');
    const orderReceiveEstimatorFacet = await OrderReceiveEstimatorFacet.deploy();
    ...
    await (await facet(flash, 'DiamondCutFacet')).diamondCut(
      [
        ...
        {
          action: FacetCutAction.Add,
          facetAddress: await orderReceiveEstimatorFacet.getAddress(),
          functionSelectors: getFunctionSelectors(orderReceiveEstimatorFacet.interface),
        },
        {
          action: FacetCutAction.Add,
      ...
      orderReceiverFacet.interface.encodeFunctionData('initializeOrderReceiverFacet', [await collateralManager.getAddress()]),
    );
  ...
  it('Should estimate order asset receive', async function () {
    ...
    // Should revert when attempted to call
    {
      let estimator = await facet(flash, 'OrderReceiveEstimatorFacet');
      estimator = estimator.connect(accounts.another);

      await expectRevert(
        estimator.estimateReceiveOrderAsset(
          order,
          orderFromSignature,
          accounts.another.address,
        ),
        { customError: 'InvalidEstimateCaller()' },
      );
    }

    // Should revert when attempted to call (from zero address)
    {
      let estimator = await facet(flash, 'OrderReceiveEstimatorFacet');
      estimator = estimator.connect(zeroSigner);
      ...
      await expectRevert(
        ...
        { customString: 'VoidSigner cannot sign transactions' },
      );
    }

    // Should provide estimate (from zero address)
    let gasEstimate: bigint;
    {
      let estimator = await facet(flash, 'OrderReceiveEstimatorFacet');
      estimator = estimator.connect(zeroSigner);

      gasEstimate = await estimator.estimateReceiveOrderAsset.estimateGas(
        order,
        orderFromSignature,
        accounts.another.address,
      );
      console.log(`estimated call receiveOrderAsset (first): ${gasEstimate}`);

      // Gas fluctuates a bit between test runs
      expect(gasEstimate).to.be.greaterThanOrEqual(114_000n);
      expect(gasEstimate).to.be.lessThanOrEqual(116_000n);
    }

    // Should provide estimate (from zero address, skip signature check)
    {
      let estimator = await facet(flash, 'OrderReceiveEstimatorFacet');
      estimator = estimator.connect(zeroSigner);

      const gas = await estimator.estimateReceiveOrderAsset.estimateGas(
        order,
        '0x', // signature
        accounts.another.address,
      );
      console.log(`estimated call receiveOrderAsset (first, skip sig): ${gas}`);

      // Gas fluctuates a bit between test runs
      expect(gas).to.be.greaterThanOrEqual(108_000n);
      expect(gas).to.be.lessThanOrEqual(111_000n);
    }

    // Ensure the real facet transaction is submittable
    {
      let receiver = await facet(flash, 'OrderReceiverFacet');
      receiver = receiver.connect(accounts.another);

      const { tx, receipt } = await gasInfo(
        'call receiveOrderAsset (first, after estimate)',
        await receiver.receiveOrderAsset(order, orderFromSignature),
      );
      expectLog({
        contract: receiver,
        tx,
      ...
      const gasEstimateExceed = gasEstimate - receipt.gasUsed;
      console.log(`estimate exceed receiveOrderAsset (first): ${gasEstimateExceed}`)

      // Ensure actually used gas does not exceed estimate
      expect(gasEstimateExceed).to.be.greaterThanOrEqual(0n);
    }

    // Ensure estimator reflects received order error
    {
      let estimator = await facet(flash, 'OrderReceiveEstimatorFacet');
      estimator = estimator.connect(zeroSigner);

      await expectRevert(
        estimator.estimateReceiveOrderAsset.estimateGas(
          order,
          orderFromSignature,
          accounts.another.address,
        ),
